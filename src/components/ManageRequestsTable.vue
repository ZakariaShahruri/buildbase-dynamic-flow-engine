...
const sortFlowInstances = (key: typeof sortKey.value) => {
  ...
  requests.value.sort((a, b) => {
    ...
    else if (key === 'Submitted By') res = compare(a.data.submittedBy.toLowerCase(), b.data.submittedBy.toLowerCase());
    else if (key === 'Start Date') res = new Date(a.data.startDate).getTime() - new Date(b.data.startDate).getTime();
    else if (key === 'End Date') res = new Date(a.data.endDate).getTime() - new Date(b.data.endDate).getTime();
    else if (key === 'Reason') res = compare(a.data.reason.toLowerCase(), b.data.reason.toLowerCase());
  ...
  <table class="w-full border-collapse text-sm">
    <thead>
      <tr class="text-left text-white bg-[#111]">
        <th v-for="key in ['Request Type','Submitted By','Start Date','End Date','Reason']" 
            :key="key" 
