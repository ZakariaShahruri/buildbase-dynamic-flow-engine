...
<template>
  ...
  <transition name="fade">
    <div v-if="burgerOpen" class="fixed inset-0 bg-black/50 z-40 md:hidden" @click.self="burgerOpen = false">
      <aside 
        class="absolute left-0 top-0 h-full w-64 bg-sidebarbg py-6 px-4 shadow-xl flex flex-col z-50"
  ...
  <aside 
    class="hidden md:sticky md:flex md:h-screen md:w-56 lg:w-64 xl:w-72 top-0 left-0 bg-sidebarbg py-5 shadow-md flex-col items-center z-30"
    aria-label="Sidebar navigation"
  >
    ...
    <nav 
      v-if="isReady" 
      class="w-full overflow-x-auto md:overflow-visible"
      aria-label="Sidebar navigation"
    >
      <ul class="mt-5">
        <li v-for="(item, index) in menuItems" :key="index" @click="selectOption(index)"
