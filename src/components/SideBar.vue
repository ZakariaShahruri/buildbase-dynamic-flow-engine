...
<template>
  ...
  <aside
    class="hidden md:sticky md:flex md:h-screen md:w-50 top-0 left-0 bg-sidebarbg py-5 shadow-md flex-col items-center z-30">
    <img src="/images/logo.png" alt="Logo" class="w-12 h-12 mx-auto block" />
    <hr class="border-t border-sidebarsecondary/30 mt-4 w-10/12" />
    <nav v-if="isReady" class="w-full overflow-x-auto md:overflow-visible">
      <ul class="mt-5">
        <li v-for="(item, index) in menuItems" :key="index" @click="selectOption(index)"
